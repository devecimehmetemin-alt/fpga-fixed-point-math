"""cocotb testbench for recip_unit. drive a sweep, compare every output to the
...
FMT = rc.RecipFormats(A_W=A_W, A_F=A_F, Y_W=Y_W, Y_F=Y_F, IDX_W=IDX_W,
...
def stimulus():
    # corners lands on every seed slice boundary
    xs = rc.corners(FMT)
...
async def test_sweep(dut):
    xs = stimulus()
    golden = rc.evaluate_fixed(xs, FMT)
    ...
    for i in range(len(xs) + LATENCY + 2):
        ...
        if int(dut.out_valid.value):
            got.append((dut.y.value.to_signed(), int(dut.ovf.value)))
    ...
    for i, (g, ref) in enumerate(zip(got, golden)):
        assert g == ref, (f"sample {i}: a={xs[i]} = {xs[i] * 2.0 ** -A_F:.9f}  "
                          f"rtl=(y={g[0]}, ovf={g[1]})  "
                          f"model=(y={ref[0]}, ovf={ref[1]})  diff={g[0] - ref[0]}")
